...
(function(root, factory){
  ...
  function weighted(parts){
    ...
    Object.keys(parts).forEach(function(k){
      var p = parts[k];
      if(p && Number.isFinite(Number(p.weight))){
  ...
  function contactability(input){
    input = input || {};
    return weighted({
      phone: {score: input.phone_verified ? 100 : input.phone_present ? 55 : 0, weight: 35},
      email: {score: input.email_verified ? 100 : input.email_present ? 45 : 0, weight: 25},
      person:{score: input.decision_maker_identified ? 100 : input.director_identified ? 65 : 0, weight: 25},
      source:{score: input.source_confidence || 0, weight: 15}
    });
  }
  ...
  function commercialScore(input){
    input = input || {};
    var mwpScore = pct((Number(input.potential_mwp || 0) / Number(input.target_mwp || 10)) * 100);
    var consumptionScore = pct(Number(input.self_consumption_pct || 0));
    var valueScore = pct((Number(input.estimated_personal_25y_commission || 0) / Number(input.target_personal_25y_commission || 250000)) * 100);
    var paybackYears = Number(input.estimated_repayment_years || 0);
    var paybackScore = paybackYears > 0 ? pct(100 - Math.max(0, paybackYears - 3) * 12.5) : 0;
    return weighted({
