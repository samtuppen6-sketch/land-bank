// LandBank V2 queued portfolio assessment worker.
// Processes a small controlled batch of sites through the existing assess-site function.

const cors = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'content-type, x-landbank-batch-key',
};
const BATCH_KEY = '4472'; // temporary single-user gate while current PIN workflow remains in use
const MAX_BATCH = 25;
...
Deno.serve(async req => {
  ...
  try {
    ...
    const batchSize = Math.max(1, Math.min(MAX_BATCH, Number(input?.batch_size || 20)));

    // Recover a worker item if a previous invocation died while processing it.
    const staleBefore = new Date(Date.now() - 15 * 60 * 1000).toISOString();
    await rest(base, service,
      `lb_assessment_queue?job_type=eq.technical_screen&status=eq.processing&locked_at=lt.${encodeURIComponent(staleBefore)}`,
    ...
    const queue: Row[] = await rest(base, service,
      `lb_assessment_queue?job_type=eq.technical_screen&status=in.(queued,failed)&attempts=lt.5&next_attempt_at=lte.${encodeURIComponent(now)}&select=id,site_id,attempts&order=attempts.asc,next_attempt_at.asc&limit=${batchSize}`
    ) || [];
    ...
    for (const item of queue) {
      ...
      try {
        const r = await fetch(`${base}/functions/v1/assess-site`, {
          ...
          body:JSON.stringify({site_id:item.site_id,providers:['pvgis','planning']})
        });
        const text = await r.text();
        let payload: any = {};
        try { payload = text ? JSON.parse(text) : {}; } catch { payload = {raw:text.slice(0,1000)}; }
        if (!r.ok || payload?.error) throw new Error(payload?.error || `assess-site ${r.status}: ${text.slice(0,500)}`);

        await patchQueue(base,service,item.id,{
          status:'completed',locked_at:null,completed_at:new Date().toISOString(),last_error:null,last_result:payload
        });
        outcome.completed++;
        outcome.items.push({site_id:item.site_id,status:'completed',site_score:payload.site_score,pvgis:payload.pvgis?.score,planning:payload.planning?.score});
      } catch (e) {
      ...
      // Polite pacing for the public planning service. PVGIS itself permits substantially more.
      await sleep(220);
    }

    const progress = await rest(base, service, 'lb_assessment_progress?select=*');
